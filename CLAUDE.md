...
- **LLM**: Vercel AI SDK (`ai`) with pluggable providers — Anthropic (`@ai-sdk/anthropic`), Ollama (`ollama-ai-provider-v2`), and OpenAI-compatible endpoints (`@ai-sdk/openai-compatible`). All access is wrapped by `src/llm/`. Models are a **named registry** in config (`models` + `default_model` + `fast_model`), resolved via `src/config/models.ts`.
- **CLI**: Commander.js
- **TUI**: Ink 7 + React 19
- **External tools**: MCPX
...
- **Named models resolve through `src/config/models.ts`.** Never index `config.models` directly — use `resolveModel(config, name?)`, `resolveFastModel(config)`, or `resolveModelFor(config, {override, pinned})`. There is no `config.llm` / `config.chunker_llm`; those keys were removed (a config still using them fails at load, by design — no aliasing).
- **Resolve models at boundaries, pass `LlmBlock` inward.** A function whose model the caller can choose takes an explicit `llm: LlmBlock` (`runChatTurn`, `runAgentLoop`); a function hard-wired to the fast role takes `BotholomewConfig` and resolves internally (`generateThreadTitle`, `evaluateSchedule`, `summarizeViaLLM`). Resolution happens in `startChatSession`, `runDream`, and `runClaimedTask` (per task, not per worker) so an unknown name fails before a thread or claim exists. Precedence is always `--model` > task/schedule `model:` > `default_model`.
...
- **Agent tools live in `src/tools/membot/`.** `adapter.ts` turns each upstream membot `Operation` into a Botholomew `ToolDefinition`; `edit.ts`, `copy.ts`, `exists.ts`, `count_lines.ts`, `pipe.ts`, and `run.ts` add the file-shaped UX our agents already know (git-hunk patches, presence checks, sandboxed TypeScript via the Run SDK, etc.) on top of membot's whole-file `write`. `run.ts` executes guest TypeScript in a QuickJS sandbox with curated `files.*` / `mcp.*` host functions — never read JSON from a large-result `lr_N` (paged splits aren't valid JSON); always go through a `logical_path`.
...
- **When to update which doc:**
  ...
  - Touching `src/config/models.ts` or the model-resolution boundaries → update the "Named models" section of `docs/architecture.md` and the model-selection tables in `docs/configuration.md`.
