...
- **Agent tools live in `src/tools/membot/`.** `adapter.ts` turns each upstream membot `Operation` into a Botholomew `ToolDefinition`; `edit.ts`, `copy.ts`, `exists.ts`, `count_lines.ts`, `pipe.ts`, and `query.ts` add the file-shaped UX our agents already know (git-hunk patches, presence checks, JSONata transforms, etc.) on top of membot's whole-file `write`. `query.ts` uses the `jsonata` package (declarative, no code execution) to reduce large JSON blobs in place — never read JSON from a large-result `lr_N` (paged splits aren't valid JSON); always go through a `logical_path`.
