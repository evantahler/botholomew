...
const FALLBACK_BY_PROVIDER: Record<LlmBlock["provider"], number> = {
  ...
  // a reasonable conversation; raise via the entry's `max_input_tokens` if needed.
  ollama: 16_000,
...
Update the model on that entry in the \`models\` block of your botholomew
config. If you believe this model *does* support tools but the probe is wrong,
set \`supports_tools: true\` on the entry to override.`;
...
export async function getMaxInputTokens(cfg: LlmBlock): Promise<number> {
  ...
  // can set the entry's `max_input_tokens` explicitly.
  ...
  if (resolved == null) {
    ...
    logger.debug(
      `Falling back to default context window (${resolved}) for ${cfg.provider}:${cfg.model}. Set \`max_input_tokens\` on the model entry to override.`,
    );
  }
