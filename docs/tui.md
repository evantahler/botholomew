...
The status bar also shows the session's active model as
`<name> · <provider>:<model>` — the entry `--model` selected, or
`default_model` when the flag is omitted:

```bash
botholomew chat --model fast   # status bar: fast · anthropic:claude-haiku-4-5-...
```

An unknown name exits with the configured names listed, before the TUI
renders. See [configuration](configuration.md) for the registry.

The bottom tab bar shows a context-usage indicator on the right
(`<used>/<max>` tokens, e.g. `146k/200k`) after the first assistant
turn lands. It's the prompt size the server billed for (input +
cache reads + cache creation) over the model's `max_input_tokens`.
...
the breakdown is a rough heuristic showing where the bytes are
going.
